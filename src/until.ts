...
export class SlDice {
  ...
  roll(empathy:number[] = null, modify:number = 0): [number, { description: string, isEmpathy: boolean, status: number, },result:number,resultText:string] {
    const index:number = D(1, this.diceCount, 1, modify, 0);
    let result:number = 0;
  ...
  copy(targetInfo: SlDice, targetIndex: number, selfIndex: number): void {
    if (targetIndex > targetInfo.diceCount || targetIndex<=0 || selfIndex > this.diceCount || selfIndex<=0 ||targetInfo.table[targetIndex].isEmpathy===false) {
      return;
    }
  ...
  toString(): string {
    ...
    for (let i = 0; i < this.diceCount; i++) {
      if (this.table[i-1].isEmpathy) {
        fmtString += `${i}:${this.table[i-1].description}\n`;
      }
    }
    return fmtString;
  }
